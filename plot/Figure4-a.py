...
CPU_Temp_dataset = pd.read_csv('workload/qh2-rcc192-MTL LSTM 7-train.csv')
CPU_Energy_dataset = pd.read_csv('workload/qh2-rcc192-MTL LSTM 8-train.csv')

# 手动设置每一列的label
CPU_Temp_dataset.columns = ['Inlet_Temperature', 'CPU_Load', 'Fan_speed1', 'Fan_speed2', 'Fan_speed3', 'Fan_speed4']
print(CPU_Temp_dataset.head())
print('CPU_Temp_dataset.shape:', CPU_Temp_dataset.shape)

CPU_Energy_dataset.columns = ['Power', 'CPU_Load', 'RAM_Load']
print(CPU_Energy_dataset.head())
print('CPU_Energy_dataset.shape:', CPU_Energy_dataset.shape)

values1 = CPU_Temp_dataset.values   # 温度相关
values1 = values1.astype('float32')

values2 = CPU_Energy_dataset.values  # 能耗相关
...
# print("缩放train:", train_scaled2)
# print("缩放test:", test_scaled2)
...
model.summary()
history = model.fit([train1X, train2X], [train1Y, train2Y], epochs=200, batch_size=32)
# print(history.history.keys()) # 查看一下history支持的输出

# history = model.fit({'input2': train2X, 'input1': train1X},
#                     {'main_output': main_output, 'aux_output': auxiliary_output},
#                     batch_size=72, epochs=10)




# x = Concatenate([model_input1, model_input2])

# 参数共享
# x = LSTM(64, return_sequences=True)(x)

# shared_lstm = LSTM(64, return_sequences=False)(model_input1)

# lstm1 = LSTM(64, return_sequences=True)(model_input1)
# lstm2 = LSTM(64, return_sequences=True)(model_input2)

# 任务一
# out1 = Attention(32)(shared_lstm)
# out1 = Dense(1, name="output1")(out1)
#
# # 任务二
# out2 = Attention(32)(shared_lstm)
# out2 = Dense(1)(out2)
#
# concatenate = Concatenate()([model_input2, shared_lstm])
# out3 = Dense(units=1, name='output3')(concatenate)
#
# model = Model(inputs=[model_input1, model_input2], outputs=[out1, out2, out3])
# fasd
#
# model.compile(loss="mae", optimizer="adam")

# history1 = model.fit(train1X, train1Y, batch_size=72, epochs=10, validation_data=(train1X, train1Y), verbose=2, shuffle=False)
...
original2_copies_array = np.repeat(test2Y, 3, axis=-1)
original2 = scaler2.inverse_transform(np.reshape(original2_copies_array, (len(test2Y), 3)))[:, 0]
print("Pred2 Values-- ", pred2)
print("\nOriginal2 Values-- ", original2)
# 计算rmse
rmse2 = sqrt(mean_squared_error(original2, pred2))
print("Test RMSE2:%.3f:", rmse2)

mylog1 = open('orig1.txt', mode='a', encoding='utf-8')
np.set_printoptions(threshold=np.inf)
for i in range(len(original1)):
    print(original1[i], end=',', file=mylog1)
mylog1.close()

mylog2 = open('pred1.txt', mode='a', encoding='utf-8')
np.set_printoptions(threshold=np.inf)
for i in range(len(pred1)):
    print(pred1[i], end=',', file=mylog2)
mylog2.close()

mylog3 = open('orig2.txt', mode='a', encoding='utf-8')
np.set_printoptions(threshold=np.inf)
for i in range(len(original2)):
    print(original2[i], end=',', file=mylog3)
mylog3.close()

mylog4 = open('pred2.txt', mode='a', encoding='utf-8')
np.set_printoptions(threshold=np.inf)
for i in range(len(pred2)):
    print(pred2[i], end=',', file=mylog4)
mylog4.close()

# plt.plot(original1, color='#BEB8DC', label='Origin Temperature')
# plt.plot(pred1, color='#FA7F6F', label='Predicted Temperature')
...
# plt.show()
#
# plt.plot(original2, color='#BEB8DC', label='Origin Energy Consumption')
# plt.plot(pred2, color='#FA7F6F', label='Predicted Energy Consumption')
...
df_30_days_future1 = pd.read_csv("workload/qh2-rcc192-MTL LSTM 7-test.csv")
...
df_30_days_future2 = pd.read_csv("workload/qh2-rcc192-MTL LSTM 8-test.csv")
...
df_origin1 = df_30_days_future1 = pd.read_csv("workload/qh2-rcc192-MTL LSTM 7.csv")
...
df_origin2 = df_30_days_future2 = pd.read_csv("workload/qh2-rcc192-MTL LSTM 8.csv")
...
# mylog1 = open('orig1.txt', mode='a', encoding='utf-8')
# np.set_printoptions(threshold=np.inf)
# for i in range(len(y_origin1)):
#     print(y_origin1[i], end=',', file=mylog1)
# mylog1.close()
#
# mylog2 = open('pred1.txt', mode='a', encoding='utf-8')
# np.set_printoptions(threshold=np.inf)
# for i in range(len(y_pred_future_30_days_1)):
#     print(y_pred_future_30_days_1[i], end=',', file=mylog2)
# mylog2.close()
